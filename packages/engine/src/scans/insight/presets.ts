 ...
 * drift run to run — the same split as `kind-presets.ts`. The genuinely shared
 * pieces (the read-only toolsets + the analyzer persona every scan reuses) live
 * in `../shared/presets.ts`; this module holds ONLY the Insight-specific parts.
 */
