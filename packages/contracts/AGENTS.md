...
- Every exported zod schema is a PascalCase const suffixed `Schema`, paired with a same-named inferred type. Do NOT hand-author a duplicate type:
  ...
  ```
  Enforced by `noctcore-contracts/zod-schema-naming` (`error` on `packages/contracts/src/**`): discriminated-union *member* schemas below use role suffixes (`Command`/`Event`/`Query`), not `Schema`, and are carved out inside the rule — their naming contract is `noctcore-contracts/wire-message-naming`.
