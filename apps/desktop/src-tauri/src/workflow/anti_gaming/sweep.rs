...
pub fn append_anti_gaming_check(
    ...
    // `--no-textconv` refuses an attacker-configured `[diff "x"] textconv=<cmd>`
    // binding — a SEPARATE host-RCE that `--no-ext-diff` does NOT cover and that has
    // no `-c` neutralizer (the driver name is attacker-chosen).
    let Some(diff) = crate::git::run::git_stdout(
        review_dir,
        &[
            "diff",
            "--no-ext-diff",
