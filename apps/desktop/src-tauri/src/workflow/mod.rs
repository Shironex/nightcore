...
//! budget, strictness ratchet), the commit-path secret gate, and the per-kind
//! run policy. Grouped here so the crate root holds only the module tree; the
//! historical `crate::{gauntlet, kind, merge, plan_approval}` paths are
//! preserved by the facade re-exports in `lib.rs` (the newer modules are
//! addressed as `crate::workflow::*` directly).
