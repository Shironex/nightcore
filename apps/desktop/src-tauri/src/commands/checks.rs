...
fn build_state(
    ...
    ArmedChecksState { checks, last_run }
}
...
fn state_for(project_path: &str) -> ArmedChecksState {
    ...
    let last = read_last_run(project_path).map(|r| (r.ran_at, r.result));
    build_state(files, last)
}
...
pub async fn run_armed_checks_now(app: AppHandle) -> Result<ArmedChecksState, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let path = active_project_path(&app)?;
        let result = crate::workflow::gauntlet_project::run(std::path::Path::new(&path));
        ...
        if let Err(e) = write_last_run(&path, &result, now_ms()) {
...
mod tests {
    ...
    fn build_state_folds_last_results_by_name() {
        ...
        let state = build_state(files, Some((1234, last)));
    ...
    fn build_state_without_a_last_run_has_no_outcomes() {
        let state = build_state(vec![file("lint", true)], None);
