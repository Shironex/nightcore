    ...
    <div className="flex flex-col gap-4 px-5 py-4">
      <header className="flex flex-col gap-2">
        <div className="flex items-baseline gap-2">
          <span className="font-mono text-[12px] text-muted-foreground">#{issue.number}</span>
        ...
        {issue.linkedPrs.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {issue.linkedPrs.map((pr) => (
              <span
                key={pr.number}
                className="inline-flex items-center gap-1 rounded-md border border-primary/40 bg-primary/[0.08] px-1.5 py-0.5 font-mono text-[10px] text-primary/90"
                title={pr.title}
              >
      ...
      <section className="flex flex-col gap-1.5">
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-[10px] text-muted-foreground/70">{UNTRUSTED_LABEL}</span>
        </div>
        {loading ? (
          ...
          <div className="flex items-center gap-2 rounded-[10px] border border-destructive/40 bg-destructive/[0.08] px-3 py-2 text-[12.5px] text-destructive">
            <AlertIcon size={14} />
            {error}
          </div>
